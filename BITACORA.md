...
- **Aislamiento de descargas temporales e importación atómica**: Las descargas temporales ahora se crean en el directorio `/tmp` local del contenedor (fuera de la carpeta de canciones) para evitar que Liquidsoap las escanee de manera incompleta y cause cortes. Al finalizar, se importan atómicamente a `music/songs/` usando una extensión temporal `.tmp` que se renombra al instante. Al arrancar el downloader se limpian automáticamente las subcarpetas huérfanas de sesiones anteriores en `/tmp`.
...
- **Temporizador de seguridad en descargas**: Se integró un watchdog (`threading.Timer`) de 270 segundos en el servidor del downloader para forzar el cierre y terminación del subproceso `spotiflac` si se queda bloqueado o rate-limited indefinidamente en algún proveedor (como Tidal). Esto libera automáticamente el lock de descargas y evita que se obstruya la cola de BullMQ.
