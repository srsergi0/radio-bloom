...
- **ffprobe multi-stage en publisher**: El publisher solo necesita `ffprobe` (no ffmpeg completo). Se reemplazó `RUN apk add ffmpeg` por una etapa multi-stage que extrae solo el binario `ffprobe` y sus librerías compartidas desde `alpine:3.20`, reduciendo la imagen final de 132 MB a ~5 MB adicionales.
