...
import { normalizeObjectSchema, safeParseAsync, getObjectShape, objectFromShape, getParseErrorMessage, getSchemaDescription, isSchemaOptional } from './zod-compat.js';
import { toJsonSchemaCompat } from './zod-json-schema-compat.js';
import { McpError, ErrorCode, assertCompleteRequestPrompt, assertCompleteRequestResourceTemplate } from '../types-base.js';
...
export class McpServer {
    constructor(serverInfo, options) {
        this._registeredResources = new Map();
        this._registeredResourceTemplates = new Map();
        this._registeredTools = new Map();
        this._registeredPrompts = new Map();
    ...
    setToolRequestHandlers() {
        ...
        this.server.setRequestHandler('tools/list', () => ({
            tools: [...this._registeredTools.entries()]
        ...
        this.server.setRequestHandler('tools/call', async (request, extra) => {
            try {
                const tool = this._registeredTools.get(request.params.name);
                if (!tool) {
    ...
    async handlePromptCompletion(request, ref) {
        const prompt = this._registeredPrompts.get(ref.name);
        if (!prompt) {
    ...
    async handleResourceCompletion(request, ref) {
        const template = [...this._registeredResourceTemplates.values()].find(t => t.resourceTemplate.uriTemplate.toString() === ref.uri);
        if (!template) {
            if (this._registeredResources.has(ref.uri)) {
    ...
    setResourceRequestHandlers() {
        ...
        this.server.setRequestHandler('resources/list', async (request, extra) => {
            const resources = [...this._registeredResources.entries()]
            ...
            for (const template of [...this._registeredResourceTemplates.values()]) {
        ...
        this.server.setRequestHandler('resources/templates/list', async () => {
            const resourceTemplates = [...this._registeredResourceTemplates.entries()].map(([name, template]) => ({
                name,
        ...
        this.server.setRequestHandler('resources/read', async (request, extra) => {
            ...
            const resource = this._registeredResources.get(uri.toString());
            if (resource) {
                if (!resource.enabled) {
    ...
    setPromptRequestHandlers() {
        ...
        this.server.setRequestHandler('prompts/list', () => ({
            prompts: [...this._registeredPrompts.entries()]
        ...
        this.server.setRequestHandler('prompts/get', async (request, extra) => {
            const prompt = this._registeredPrompts.get(request.params.name);
            if (!prompt) {
    ...
    resource(name, uriOrTemplate, ...rest) {
        ...
        if (typeof uriOrTemplate === 'string') {
            if (this._registeredResources.has(uriOrTemplate)) {
            ...
            if (this._registeredResourceTemplates.has(name)) {
    ...
    _createRegisteredResource(name, title, uri, metadata, readCallback) {
        const registeredResource = {
            ...
            update: updates => {
                if (typeof updates.uri !== 'undefined' && updates.uri !== uri) {
                    ...
                    if (updates.uri)
                        this._registeredResources.set(updates.uri, registeredResource);
                }
        ...
        this._registeredResources.set(uri, registeredResource);
        return registeredResource;
    }
    _createRegisteredResourceTemplate(name, title, template, metadata, readCallback) {
        const registeredResourceTemplate = {
            ...
            update: updates => {
                if (typeof updates.name !== 'undefined' && updates.name !== name) {
                    this._registeredResourceTemplates.delete(name);
                    if (updates.name)
                        this._registeredResourceTemplates.set(updates.name, registeredResourceTemplate);
                }
                if (typeof updates.title !== 'undefined')
        ...
        this._registeredResourceTemplates.set(name, registeredResourceTemplate);
    ...
    _createRegisteredPrompt(name, title, description, argsSchema, callback) {
        const registeredPrompt = {
            ...
            update: updates => {
                if (typeof updates.name !== 'undefined' && updates.name !== name) {
                    ...
                    if (updates.name)
                        this._registeredPrompts.set(updates.name, registeredPrompt);
                }
                if (typeof updates.title !== 'undefined')
        ...
        this._registeredPrompts.set(name, registeredPrompt);
        // If any argument uses a Completable schema, enable completions capability
        if (argsSchema) {
            const hasCompletable = Object.values(argsSchema).some(field => {
                // Duck-type check for ZodOptional (avoids importing ZodOptional from 'zod')
                const isOptional = (field._def?.typeName === 'ZodOptional') || (field._zod?.def?.type === 'optional');
                const inner = isOptional ? (field._def?.innerType ?? field._zod?.def?.innerType) : field;
    ...
    _createRegisteredTool(name, title, description, inputSchema, outputSchema, annotations, execution, _meta, handler) {
        ...
        const registeredTool = {
            ...
            update: updates => {
                if (typeof updates.name !== 'undefined' && updates.name !== name) {
                    ...
                    if (updates.name)
                        this._registeredTools.set(updates.name, registeredTool);
                }
                if (typeof updates.title !== 'undefined')
        ...
        this._registeredTools.set(name, registeredTool);
