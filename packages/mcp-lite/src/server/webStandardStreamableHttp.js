 ...
 */
import { isInitializeRequest, isJSONRPCErrorResponse, isJSONRPCRequest, isJSONRPCResultResponse, SUPPORTED_PROTOCOL_VERSIONS, DEFAULT_NEGOTIATED_PROTOCOL_VERSION } from '../types-base.js';
// Lazy-loaded Zod schema
let _JSONRPCMessageSchema;
async function getJSONRPCMessageSchema() {
    if (!_JSONRPCMessageSchema) _JSONRPCMessageSchema = (await import('../types.js')).JSONRPCMessageSchema;
    return _JSONRPCMessageSchema;
}
/**
...
export class WebStandardStreamableHTTPServerTransport {
    ...
    async handlePostRequest(req, options) {
        try {
            ...
            try {
                const MsgSchema = await getJSONRPCMessageSchema();
                if (Array.isArray(rawMessage)) {
                    messages = rawMessage.map(msg => MsgSchema.parse(msg));
                }
                else {
                    messages = [MsgSchema.parse(rawMessage)];
                }
            }
            catch {
