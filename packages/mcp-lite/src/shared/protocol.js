...
import { ErrorCode, isJSONRPCErrorResponse, isJSONRPCRequest, isJSONRPCResultResponse, isJSONRPCNotification, McpError, RELATED_TASK_META_KEY, isTaskAugmentedRequestParams } from '../types-base.js';
import { getMethodLiteral, parseWithCompat } from '../server/zod-json-schema-compat.js';
// Lazy-loaded Zod schemas from types.js (only loaded when first message arrives)
let _types;
function T() { return _types || (_types = import('../types.js')); }
// Inlined from experimental/tasks/interfaces.js — avoids loading the experimental barrel
...
export class Protocol {
    constructor(_options) {
        ...
        // Core maps — always needed (used in constructor)
        this._requestHandlers = new Map();
        this._notificationHandlers = new Map();
        // Lazy maps — only created when first accessed
        this._requestHandlerAbortControllers = undefined;
        this._responseHandlers = undefined;
        this._progressHandlers = undefined;
        this._timeoutInfo = undefined;
        this._pendingDebouncedNotifications = undefined;
        this._taskProgressTokens = undefined;
        this._requestResolvers = undefined;
                        ...
                        while ((queuedMessage = await this._taskMessageQueue.dequeue(taskId, extra.sessionId))) {
                            ...
                            if (queuedMessage.type === 'response' || queuedMessage.type === 'error') {
                                ...
                                // Lookup resolver in _requestResolvers map
                                    const resolver = this._getRequestResolvers().get(requestId);
                                if (resolver) {
                                    // Remove resolver from map after invocation
                                    this._getRequestResolvers().delete(requestId);
    ...
    // Lazy-init getters — Maps created on first access to save memory at startup
    _getRequestHandlerAbortControllers() {
        if (!this._requestHandlerAbortControllers)
            this._requestHandlerAbortControllers = new Map();
        return this._requestHandlerAbortControllers;
    }
    _getResponseHandlers() {
        if (!this._responseHandlers)
            this._responseHandlers = new Map();
        return this._responseHandlers;
    }
    _getProgressHandlers() {
        if (!this._progressHandlers)
            this._progressHandlers = new Map();
        return this._progressHandlers;
    }
    _getTimeoutInfo() {
    ...
    _getPendingDebouncedNotifications() {
        if (!this._pendingDebouncedNotifications)
            this._pendingDebouncedNotifications = new Set();
        return this._pendingDebouncedNotifications;
    }
    _getTaskProgressTokens() {
        if (!this._taskProgressTokens)
            this._taskProgressTokens = new Map();
        return this._taskProgressTokens;
    }
    _getRequestResolvers() {
        if (!this._requestResolvers)
            this._requestResolvers = new Map();
        return this._requestResolvers;
    }
    async _oncancel(notification) {
        ...
        const controller = this._getRequestHandlerAbortControllers().get(notification.params.requestId);
        controller?.abort(notification.params.reason);
    }
    _setupTimeout(messageId, timeout, maxTotalTimeout, onTimeout, resetTimeoutOnProgress = false) {
        this._getTimeoutInfo().set(messageId, {
    ...
    _resetTimeout(messageId) {
        const info = this._getTimeoutInfo().get(messageId);
        if (!info)
            return false;
        const totalElapsed = Date.now() - info.startTime;
        if (info.maxTotalTimeout && totalElapsed >= info.maxTotalTimeout) {
            this._getTimeoutInfo().delete(messageId);
    ...
    _onclose() {
        const responseHandlers = this._getResponseHandlers();
        this._responseHandlers = new Map();
        if (this._progressHandlers)
            this._getProgressHandlers().clear();
        if (this._taskProgressTokens)
            this._getTaskProgressTokens().clear();
        if (this._pendingDebouncedNotifications)
            this._getPendingDebouncedNotifications().clear();
        if (this._timeoutInfo) {
            for (const info of this._getTimeoutInfo().values()) {
                clearTimeout(info.timeoutId);
            }
            this._getTimeoutInfo().clear();
        }
    ...
    _onrequest(request, extra) {
        ...
        this._getRequestHandlerAbortControllers().set(request.id, abortController);
        ...
        Promise.resolve()
            ...
            if (this._getRequestHandlerAbortControllers().get(request.id) === abortController) {
                this._getRequestHandlerAbortControllers().delete(request.id);
            }
        });
    }
    _onprogress(notification) {
        ...
        const handler = this._getProgressHandlers().get(messageId);
        if (!handler) {
            this._onerror(new Error(`Received a progress notification for an unknown token: ${JSON.stringify(notification)}`));
            return;
        }
        const responseHandler = this._getResponseHandlers().get(messageId);
        const timeoutInfo = this._getTimeoutInfo().get(messageId);
        if (timeoutInfo && responseHandler && timeoutInfo.resetTimeoutOnProgress) {
            try {
                ...
                this._getResponseHandlers().delete(messageId);
                this._getProgressHandlers().delete(messageId);
    ...
    _onresponse(response) {
        ...
        const resolver = this._getRequestResolvers().get(messageId);
        if (resolver) {
            this._getRequestResolvers().delete(messageId);
        ...
        const handler = this._getResponseHandlers().get(messageId);
        if (handler === undefined) {
        ...
        if (isJSONRPCResultResponse(response) && response.result && typeof response.result === 'object') {
            ...
            if (result.task && typeof result.task === 'object') {
                ...
                if (typeof task.taskId === 'string') {
                    isTaskResponse = true;
                    this._getTaskProgressTokens().set(task.taskId, messageId);
                }
            }
        }
        if (!isTaskResponse) {
    ...
    async *requestStream(request, resultSchema, options) {
        ...
        try {
            ...
            const createResult = await this.request(request, (await T()).CreateTaskResultSchema, options);
    ...
    request(request, resultSchema, options) {
        ...
        return new Promise((resolve, reject) => {
            ...
            if (options?.onprogress) {
                this._getProgressHandlers().set(messageId, options.onprogress);
            ...
            this._getResponseHandlers().set(messageId, response => {
            ...
            if (relatedTaskId) {
                ...
                this._getRequestResolvers().set(messageId, responseResolver);
                this._enqueueTaskMessage(relatedTaskId, {
                    type: 'request',
    ...
    async getTask(params, options) {
        ...
        return this.request({ method: 'tasks/get', params }, (await T()).GetTaskResultSchema, options);
    }
    /**
    ...
    async listTasks(params, options) {
        ...
        return this.request({ method: 'tasks/list', params }, (await T()).ListTasksResultSchema, options);
    }
    /**
    ...
    async cancelTask(params, options) {
        ...
        return this.request({ method: 'tasks/cancel', params }, (await T()).CancelTaskResultSchema, options);
    }
    /**
    ...
    async notification(notification, options) {
        ...
        if (canDebounce) {
            ...
            if (this._getPendingDebouncedNotifications().has(notification.method)) {
                return;
            }
            // Mark this notification type as pending.
            this._getPendingDebouncedNotifications().add(notification.method);
            // Schedule the actual send to happen in the next microtask.
            // This allows all synchronous calls in the current event loop tick to be coalesced.
            Promise.resolve().then(() => {
                // Un-mark the notification so the next one can be scheduled.
                this._getPendingDebouncedNotifications().delete(notification.method);
    ...
    setRequestHandler(requestSchema, handler) {
        ...
        if (typeof requestSchema === 'string') {
            ...
            this._requestHandlers.set(method, (request, extra) => {
                return Promise.resolve(handler(request, extra));
            });
        } else {
            method = getMethodLiteral(requestSchema);
            this.assertRequestHandlerCapability(method);
            this._requestHandlers.set(method, (request, extra) => {
    ...
    _cleanupTaskProgressHandler(taskId) {
        const progressToken = this._getTaskProgressTokens().get(taskId);
        if (progressToken !== undefined) {
            this._getProgressHandlers().delete(progressToken);
            this._getTaskProgressTokens().delete(taskId);
        }
    }
    /**
    ...
    requestTaskStore(request, sessionId) {
        ...
        return {
            ...
            storeTaskResult: async (taskId, status, result) => {
                ...
                const task = await taskStore.getTask(taskId, sessionId);
                if (task) {
                    const notification = (await T()).TaskStatusNotificationSchema.parse({
